...
for i_continent in continent_list: 
    ...
    for river_name_s in river_darray_full_s.values:
        parts = river_name_s.split("; ")
        cleaned = [
            re.sub(r'\s*\(.*?\)', '', p).lower()
            for p in parts
        ]
        river_clean_full_s.append(cleaned)
    ...
    for station_id in list_station_id:
        ...
        result, reason = utils.corresp_name_dist_area(station_id, river_clean_full_s, r_id_swr, area_darray_g, river_darray_g, facc_darray_full_s, dschg_swt, x_found_station, y_found_station, geox_darray_full_s, geoy_darray_full_s)
        if reason == 'no match name':
            no_match_name += 1
        if reason == 'no match areas':
            no_match_area += 1
...
# Save to netcdf
dset_global.to_netcdf("/obs/ecastonguay/scripts/global_dset_v4.nc")